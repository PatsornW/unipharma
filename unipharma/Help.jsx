...
function HelpPage({ lang, L, perm = { role: 'admin' } }) {
  ...
  const PAGES = [
    ...
    { icon:'💊', th:'ฐานข้อมูลยา', en:'Drug Database', color:'var(--ok)',
      ...
      steps_en:['Search by code/name; filter by category/VAT/branch (pick a branch → only drugs stocked there)','View full details: cost incl. VAT (top) / excl. VAT (below), profit%, packaging, stock','Add new or edit drugs (profit editable → auto-update sell)','Toggle Packaging ON to see packaging hierarchy','Edit packaging in the Edit form','🏷️ "Categories" button: add/edit/delete main & sub-categories (bilingual), saved & shared to the cloud','🏪 Stock CW column shows live CW Pharma stock per branch (PTN/RAM/CNX) — green>10, yellow>0, red=out; expand row to see cost/sell per branch','📤 Admin/Manager: tap Export Excel to download the full drug database as an Excel file'] },
    { icon:'📋', th:'การสั่งซื้อ', en:'Purchase Orders', color:'var(--info)',
      steps_th:['ดูรายการ PO ทั้งหมด กรองตามสาขา/สถานะ/เดือน','เปลี่ยนสถานะ: ส่ง → อนุมัติ → ยืนยันรับ','ดูเอกสาร A4 (ชื่อ Supplier & จำนวนเงินแปลเป็นคำ)', 'สร้าง PO ใหม่: เลือกสาขา+Supplier → ดึงราคา → เลือกสินค้า','⚠️ แจ้งเตือนราคา: ระบบเปรียบเทียบราคาที่กรอกกับประวัติ 90 วัน — เหลือง=สูงกว่าราคาดีที่สุด, เขียว=ต่ำกว่าค่าเฉลี่ย (ช่วยประหยัดงบ)','สถานที่จัดส่ง: เลือกสาขาจาก Dropdown → ที่อยู่เติมอัตโนมัติ (แก้ไขได้) พร้อมเวลาเปิด+เบอร์โทร','เลือกดีล/โปรโมชั่นของ Supplier — ใช้ข้อมูลล่าสุดเสมอ','🏪 แถบ CW ใต้รายการยา: สต็อก 3 สาขาจาก CW + เปรียบเทียบราคา PO vs ต้นทุน CW','📤 Admin/Manager: Export Excel เพื่อดาวน์โหลดรายการ PO ทั้งหมด'],
      steps_en:['View all POs, filter by branch/status/month','Change status: submit → approve → confirm','View A4 document (supplier name & amount in words translated)','Create PO: select branch+supplier → auto prices → select items','⚠️ Price alert: price field shows yellow if above 90-day best price, green if below average — helps avoid overpaying','Delivery: pick a branch from the dropdown → address auto-fills (editable) with open hours + phone','Pick a supplier deal/promotion — always uses the latest data','🏪 CW strip below each item: stock per branch from CW Pharma + PO price vs CW cost comparison','📤 Admin/Manager: Export Excel to download all purchase order records'] },
    { icon:'🏭', th:'ผู้จัดจำหน่าย', en:'Suppliers', color:'var(--warn)',
      steps_th:['ดูการ์ด Supplier พร้อมยอดซื้อ คะแนน','คลิกการ์ดเพื่อดูรายการยา ประวัติ PO โปรโมชั่น','แก้ไขข้อมูล (ชื่อ/ติดต่อ/เครดิต) หรือเพิ่มผู้จัดจำหน่ายใหม่','เพิ่ม/แก้ไข/ลบโปรโมชั่นในฟอร์มแก้ไข (ชื่อ + ส่วนลด% + ใช้ได้ถึง) → ใช้ตอนสร้าง PO ได้ทันที','📤 Admin/Manager: Export Excel เพื่อดาวน์โหลดรายการผู้จัดจำหน่าย'],
      steps_en:['View supplier cards with spend, rating, promotions','Click card to see drug list, PO history, deals','Edit info or add new supplier','Add/edit/remove promotions in the edit form (name + discount% + valid-until) → available immediately when creating a PO','📤 Admin/Manager: Export Excel to download the full supplier list'] },
    ...
    { icon:'🔄', th:'ซิงค์ข้อมูล', en:'Data Sync', color:'var(--info)',
      steps_th:['Admin เท่านั้น — นำเข้ายา/ผู้จัดจำหน่าย/ออเดอร์ ผ่าน Excel หรือ Google Sheets','จับคู่คอลัมน์อัตโนมัติ → Preview → Import (Merge กับข้อมูลเดิม ไม่ลบทับ)','ดาวน์โหลด Template .xlsx พร้อม dropdown สำหรับกรอกข้อมูลได้เลย','SQL Sync: รัน SQL snippets เพื่ออัปเกรดตาราง (ทำครั้งแรกครั้งเดียว)','Startup Query: SQL ที่รันอัตโนมัติทุกครั้งที่เปิดแอป (warehouse sync)','ประวัติการ sync แสดงด้านล่าง — บันทึกวันเวลาและจำนวนรายการที่นำเข้า'],
      steps_en:['Admin only — import drugs/suppliers/orders via Excel or Google Sheets','Auto-map columns → preview → import (merges, never overwrites existing data)','Download .xlsx template with dropdowns ready to fill','SQL Sync: run SQL snippets to upgrade tables (one-time setup)','Startup Query: SQL that runs automatically every time the app opens','Import history shown below — logs date, time, and record counts'] },
  ];
