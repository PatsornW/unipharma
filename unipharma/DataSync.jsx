...
function DataSyncPage({ lang, L, drugs, setDrugs, suppliers, setSuppliers, notify, perm = { canWrite: true } }) {
  ...
  const SQL_SNIPPETS = [
    {
      labelTH: '📈 สร้างตาราง price_history (ประวัติราคาสินค้า)',
      labelEN: '📈 Create price_history table (purchase price tracking)',
      sql: `-- สร้างตาราง price_history สำหรับบันทึกราคาซื้อทุก PO ที่อนุมัติ\nCREATE TABLE IF NOT EXISTS price_history (\n  id          uuid DEFAULT gen_random_uuid() PRIMARY KEY,\n  drug_code   text NOT NULL,\n  supplier_id text,\n  cost_ex     numeric NOT NULL DEFAULT 0,\n  po_number   text,\n  po_date     date,\n  recorded_at timestamptz DEFAULT now(),\n  recorded_by text\n);\n\nCREATE INDEX IF NOT EXISTS idx_price_history_drug   ON price_history(drug_code);\nCREATE INDEX IF NOT EXISTS idx_price_history_sup    ON price_history(supplier_id, drug_code);\nCREATE INDEX IF NOT EXISTS idx_price_history_date   ON price_history(recorded_at DESC);\n\n-- เปิด Realtime สำหรับตารางนี้ด้วย\nALTER TABLE price_history REPLICA IDENTITY FULL;\n\n-- (Optional) เปิด Row-Level Security แบบ read-all สำหรับ authenticated users\nALTER TABLE price_history ENABLE ROW LEVEL SECURITY;\nCREATE POLICY IF NOT EXISTS "read all" ON price_history FOR SELECT TO authenticated USING (true);\nCREATE POLICY IF NOT EXISTS "insert auth" ON price_history FOR INSERT TO authenticated WITH CHECK (true);`,
    },
    {
    ...
    {
      labelTH: '🔴 เปิด Realtime Sync ทุกตาราง (รันครั้งเดียว)',
      labelEN: '🔴 Enable Realtime Sync for all tables (run once)',
      sql: `-- เปิด Realtime ให้ทุกตาราง — ทำให้ข้อมูลอัปเดตอัตโนมัติเมื่อผู้ใช้คนอื่นแก้ไข\n-- รันใน Supabase Dashboard → SQL Editor\nALTER TABLE drugs            REPLICA IDENTITY FULL;\nALTER TABLE suppliers        REPLICA IDENTITY FULL;\nALTER TABLE purchase_orders  REPLICA IDENTITY FULL;\nALTER TABLE out_of_stock     REPLICA IDENTITY FULL;\nALTER TABLE categories       REPLICA IDENTITY FULL;\n\nALTER PUBLICATION supabase_realtime ADD TABLE drugs;\nALTER PUBLICATION supabase_realtime ADD TABLE suppliers;\nALTER PUBLICATION supabase_realtime ADD TABLE purchase_orders;\nALTER PUBLICATION supabase_realtime ADD TABLE out_of_stock;\nALTER PUBLICATION supabase_realtime ADD TABLE categories;`,
    },
    {
